...
import { getProjectObject, newProject, projectList, projects } from './index.js';
import "./styles.css";
...
export function submitForm(formData, reliable) {
    ...
    itemCreationForm.addEventListener("submit", (ev) => {
        ...
        addTodoItem(todoList.id, (Array.from(newFormData)).map((n) => n[1]));
...
export function addTodoItem(listID, properties, isComplete, checklist = "banana") {
    ...
    */
    ...
    let thisItemObject = (properties[5] === "basic") ? todoListItem.todos[todoListItem.add(
        new TodoItem(properties[0], properties[1], properties[2], properties[3], properties[4], properties[5], todoItemDiv
        ))] : todoListItem.todos[todoListItem.add(
            new ChecklistItem(properties[0], properties[1], properties[2], properties[3], properties[4], properties[5], todoItemDiv
            ))];
    ...
    if (checklist != "banana") {
        checklist.forEach((n) => {
            createCheckmarkItem(todoItemDescChecklist, n.name, thisItemObject, n.done);
        })
    }
...
function createCheckmarkItem(todoItemDesc, desc, object, done = false) {
    ...
    checkmarkDesc.addEventListener("click", () => {
        ...
        if (newName != null) { checkmarkDesc.textContent = newName; thisCheckItem.name = newName; }
        localStorage.setItem("userData", getProjectObject());
    });
    const checkmarkButton = document.createElement("div");
    const destructionButton = document.createElement("button");
    const thisCheckItem = new CheckItem(checkmarkDesc.textContent, checkmarkButton);
